...
void init_random_config(struct act_ptl *ptls, const int lx, const int ly, const int csize, const int ptlsNum){
	...
	for (int i=0;i<ptlsNum;i++) {
        ...
        ptls[i].px = (float) lx * ((double) rand() / RAND_MAX);
        ptls[i].py = (float) ly * ((double) rand() / RAND_MAX);
...
__global__ void move(struct act_ptl *ptls, float *chem, curandState *state, int *cellHead, int *cellTail, const int lx, const int ly, const int csize, const int ptlsNum, const float dt, const float zeta, const float D0, const float Dr)
{
    const int tid = threadIdx.x + blockDim.x * blockIdx.x;
    if(tid < ptlsNum) {
        ...
        v0 = chem[pos];
        v = (float) (v0 - zeta * r_loc > 0.0) * (v0 - zeta * r_loc);
         
        ptls[tid].vx = v * cosf(theta) + sqrt(2.0 *D0 / dt) *(2.*curand_uniform(&state[tid])-1.0);
        ptls[tid].vy = v * sinf(theta) + sqrt(2.0 *D0 / dt) *(2.*curand_uniform(&state[tid])-1.0) ;
        
        //updating X
        ptls[tid].px += dt * (ptls[tid].vx);
        // ptls[tid].px += dt * ptls[tid].vx; // no thermal diffusion
        if(ptls[tid].px < 0.0)    ptls[tid].px += lx ;
        if(ptls[tid].px >= lx) ptls[tid].px -= lx ;

        //updating Y
        ptls[tid].py += dt * (ptls[tid].vy);
        // ptls[tid].py += dt * ptls[tid].vy; // no thermal diffusion
...
__global__ void chem_update_rhon(float *chem, int *cellHead, int *cellTail, const int lx, const int ly, const int csize, const int cllsNum, const float dt, const float Dc, const float n0, const float lm, const float rho, float inj)
{
    const int tid = threadIdx.x + blockDim.x * blockIdx.x ;
    if(tid<cllsNum) {
...
__global__ void chem_update_rhovn(struct act_ptl *ptls, float *chem, int *cellHead, int *cellTail, const int lx, const int ly, const int csize, const int cllsNum, const float dt, const float Dc, const float n0, const float lm, const float rho, float inj)
{
    const int tid = threadIdx.x + blockDim.x * blockIdx.x ;
    if(tid<cllsNum) {
