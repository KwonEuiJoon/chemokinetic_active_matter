...
int main(int argc, char *argv[]){
    ...
    const float  u0 = 100.0;
    const float  n0     = atof(argv[7]);
    const float  lm     = atof(argv[8]);
    const int    mode   = atoi(argv[9]);
    
    float inj;
    if (mode == 0) { inj = 0.0; }
    else if (mode == 1) {inj = lm * rho * n0; }
    else if (mode == 2) {inj = lm * n0 * rho * (0.55*n0 + 0.13); }
    ...
    fprintf(fp, "Lx Ly tmax csize dt rho u0 n0 lm D0 Dr Dc\n");
    fprintf(fp, "%d %d %d %d %f %f %f %f %f %f %f %f", Lx_size, Ly_size, tmax, csize, dt, rho, u0, n0, lm, D0, Dr, Dc);
    ...
    if (argc == 12) 
        {   char saved_ptls_name[200];
            ...
            sprintf(saved_ptls_name, "%s/state_t_%d.txt", argv[10], atoi(argv[11]));
            sprintf(saved_chem_name, "%s/chem_t_%d.txt", argv[10], atoi(argv[11]));
    ...
    for(int t = 0; t<=tmax; t++) {
        ...
        // particle move
        move<<<nBlocks, nThreads>>>(devPtls, devChem, devStates, devHead, devTail, Lx_size, Ly_size, csize, ptlsNum, dt, u0, D0, Dr);
        // chemical update
        if (mode == 1) {chem_update_BMR<<<nBlocks2, nThreads2>>>(devChem, devHead, devTail, Lx_size, Ly_size, csize, cllsNum, dt, Dc, n0, lm, rho, inj); }
        if (mode == 2) {chem_update_AMR<<<nBlocks2, nThreads2>>>(devPtls, devChem, devHead, devTail, Lx_size, Ly_size, csize, cllsNum, dt, Dc, n0, lm, rho, inj); }
    ...
    cudaFree(devPtls); cudaFree(devChem); cudaFree(devStates); cudaFree(devCell); cudaFree(devHead); cudaFree(devTail);
